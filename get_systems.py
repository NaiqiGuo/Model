...
if __name__ == "__main__":
    ...
    for STRUCTURE in ["frame", "bridge"]: # ["frame"], ["bridge"] if we only want one
        for ELASTIC in [True, False]: # [True], [False] if we only want one
                ...
                for source in [elastic_name, "field"]:
                    for quantity in quantities:
                        try:
                            ...
                            if WINDOWED:
                                sig = np.asarray(sid_outputs[0]).copy()
